...
validloader = torch.utils.data.DataLoader(testset, batch_size=args.test_batch_size,
                                         sampler=SubsetRandomSampler(range(0, 5000)), num_workers=2)


testloader = torch.utils.data.DataLoader(testset, batch_size=args.test_batch_size,
                                         sampler=SubsetRandomSampler(range(5000, 10000)), num_workers=2)
...
def valid(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(validloader):
        ...
        progress_bar(batch_idx, len(validloader), 'VALID - Loss: %.3f | Acc: %.3f%% (%d/%d)'
